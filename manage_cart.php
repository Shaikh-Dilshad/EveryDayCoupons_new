...
if($_SERVER["REQUEST_METHOD"]=="POST")  
{
    if(isset($_POST["Add_to_cart"]))
    {
     if(isset($_SESSION["cart"]))   // if season has some value it will run if not then else value will run which set the seson//seaspn is like localstorage which store the data permentaly
     {
       ...
       $_SESSION['cart'][$count]=array('item_name'=>$_POST['item_name'],'item_img'=>$_POST['item_img'],'price'=>$_POST['price'],'product_id'=>$_POST['product_id']);
       echo "<script>
       alert('Item added')
       window.location.href = 'index.php'
       </script>";
     ...
     else
     {
        // -----thi is key----//        // -----thi is value----//
            //  |                            //  |
            //  |                            //  |
        $_SESSION['cart'][0]=array('item_name'=>$_POST['item_name'],'item_img'=>$_POST['item_img'],'price'=>$_POST['price'],'product_id'=>$_POST['product_id']); //here we arre setting the season name is cart with values
        echo "<script>
        alert('Item added')
        window.location.href = 'index.php'
        </script>";
      ...
      if (isset($_POST["view_details"])) {
        $_SESSION['details'][0]=array('item_name'=>$_POST['item_name'],'item_img'=>$_POST['item_img'],'price'=>$_POST['price']); //here we arre setting the season name with values
